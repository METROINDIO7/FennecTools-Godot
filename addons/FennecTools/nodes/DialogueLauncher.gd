...
func start() -> void:
	...
	print("Estado global antes de iniciar: FGGlobal.talk = ", FGGlobal.talk if typeof(FGGlobal) != TYPE_NIL else "N/A")
	
	# ✨ SIMPLIFICADO: Solo verificar el estado global
	if typeof(FGGlobal) != TYPE_NIL and FGGlobal.talk:
		push_warning("DialogueLauncher: Ya hay un diálogo activo. Ignorando inicio.")
		return
	
	# ✨ SIMPLIFICADO: Activar estado global inmediatamente
	if typeof(FGGlobal) != TYPE_NIL:
		FGGlobal.talk = true
	...
	if slots.size() == 0:
		# ✨ SIMPLIFICADO: Limpiar estado global si no hay slots
		if typeof(FGGlobal) != TYPE_NIL:
			FGGlobal.talk = false
		finished.emit()
		return
	...
	# ✨ NUEVO: Usar un bloque de control para asegurar la limpieza
	...
	for slot_idx in range(slots.size()):
		...
		if slot.instance_scene:
			...
			if instance_success and _current_instance and _current_instance.has_method("start_dialogue_flow"):
		...
		for item_idx in range(items.size()):
			...
			# Obtener y crear panel si es necesario
			...
			if current_panel:
				...
				# Configurar expresión del personaje
				if _current_character_group != "" and slot.expression_id >= 0:
					animate_character_for_dialogue(_current_character_group, slot.expression_id, current_panel)
				...
				# ✨ CRÍTICO: Si es el último item, limpiar estado global
				if is_last_item:
					dialogue_completed = true
					if typeof(FGGlobal) != TYPE_NIL:
						FGGlobal.talk = false
					print("=== DIÁLOGO COMPLETADO ===")
					print("Estado global después de completar: FGGlobal.talk = ", FGGlobal.talk)
	
	# Limpieza final de paneles
	if current_panel and is_instance_valid(current_panel):
		if current_panel.has_method("request_exit"):
			await current_panel.request_exit()
		current_panel.queue_free()
	...
	# ✨ ASEGURAR: Limpiar estado global si no se hizo antes
	if not dialogue_completed and typeof(FGGlobal) != TYPE_NIL:
		FGGlobal.talk = false
		print("=== DIÁLOGO CANCELADO O INTERRUMPIDO ===")
		print("Estado global limpiado: FGGlobal.talk = ", FGGlobal.talk)
	
	finished.emit()

func cancel():
	_cancelled = true
	# ✨ SIMPLIFICADO: Solo limpiar estado global
	if typeof(FGGlobal) != TYPE_NIL:
		FGGlobal.talk = false
	print("=== DIÁLOGO CANCELADO ===")
	print("Estado global después de cancelar: FGGlobal.talk = ", FGGlobal.talk)
...
func _handle_slot_instance(slot: DialogueSlotConfig) -> bool:
	if not slot or not slot.instance_scene:
		return false
	_cleanup_current_instance()
	var instance_parent = _get_panel_parent()
...
func animate_character_for_dialogue(character_group_name: String, expression_id: int, dialog_panel: Node = null) -> bool:
	if character_group_name.is_empty() or expression_id < 0:
		return false
